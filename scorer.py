...
def compute_accuracy(ans_stats, wnli_output=None, sort=True):
    other_by_guid={}
    target_by_guid={}
    gold_by_guid={}
    for guid,ex_true,loss in ans_stats:
    ...
    if not wnli_output is None:
        pickle.dump( [target_by_guid, gold_by_guid, other_by_guid], open( wnli_output+".tmp", "wb" ) )
    ...
    for guid,gold in gold_by_guid.items():
        if gold=="err_true" or gold=="err_false":
            ...
            if (guid in other_by_guid and other_by_guid[guid] <= target_by_guid[guid]) or np.isnan(target_by_guid[guid]) or np.isinf(target_by_guid[guid]):
                n_overall+=1
                output.append((guid,"0"))
            else:
                n_correct+=1
...
def scorer(ans_stats, test_set, output_file=None):
    ...
    elif test_set in ["dpr-test","dpr-dev-small","wscr-test","pdp","winogender","winobias-pro1","winobias-pro2","winobias-anti1","winobias-anti2","winobias-dev","wikicrem-dev", "knowref-test"]:
        return compute_accuracy(ans_stats)
    elif test_set in [ "winogrande-test", "wsc", "winogrande-dev"]:
        return compute_accuracy(ans_stats,wnli_output=output_file, sort=False)
